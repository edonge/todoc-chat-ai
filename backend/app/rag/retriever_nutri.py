...
class NutriRetriever:
    ...
    def _ensure_loaded(self):
        ...
        index_path = _resolve_path(settings.RAG_INDEX_PATH_NUTRI)
        meta_path = _resolve_path(settings.RAG_META_PATH_NUTRI)

        if not index_path.exists() or not meta_path.exists():
            raise FileNotFoundError("Nutrient RAG 인덱스 또는 메타데이터가 없습니다. ingest_nutri 스크립트를 먼저 실행하세요.")
