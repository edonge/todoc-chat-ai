...
class CommonRetriever:
    ...
    def _ensure_loaded(self):
        ...
        index_path = _resolve_path(settings.RAG_INDEX_PATH_COMMON)
        meta_path = _resolve_path(settings.RAG_META_PATH_COMMON)

        if not index_path.exists() or not meta_path.exists():
            raise FileNotFoundError("Common RAG 인덱스 또는 메타데이터가 없습니다. ingest_common 스크립트를 먼저 실행하세요.")
    ...
    def search(self, query: str, top_k: int = 3, score_threshold: float = 0.25) -> str:
        ...
        for score, meta in results:
            formatted.append(
                f"- [{meta.get('source','p')} p{meta['page']}:{meta['chunk_id']}] (score {score:.2f}) {meta['content']}"
            )
        return "\n".join(formatted)
