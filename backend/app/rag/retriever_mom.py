...
class MomRetriever:
    ...
    def _ensure_loaded(self):
        ...
        index_path = _resolve_path(settings.RAG_INDEX_PATH_MOM)
        meta_path = _resolve_path(settings.RAG_META_PATH_MOM)

        if not index_path.exists() or not meta_path.exists():
            raise FileNotFoundError("Mom RAG 인덱스 또는 메타데이터가 없습니다. ingest_mom 스크립트를 먼저 실행하세요.")
